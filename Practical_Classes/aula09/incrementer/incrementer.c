...
int main (int argc, char *argv[])
{
    ...
    for (i = 0; i < nthr; i++) {
        if (pthread_create (thr+i, NULL, incrementer, &niter) != 0) { // pthread(array para guardar os valores, NULL, a sua ação, num do processo)
    ...
    for (i = 0; i < nthr; i++) { 
        if (pthread_join (thr[i], (void **) &status_p) != 0) { // join espera que as threads no array terminem
            fprintf (stderr, "thread %d\n", i);
